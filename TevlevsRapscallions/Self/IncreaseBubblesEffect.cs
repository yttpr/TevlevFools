...
// Type: TevlevsRapscallions.IncreaseBubblesEffect
...
{
  public class IncreaseBubblesEffect : EffectSO
  {
    [SerializeField]
      ...
      out int exitAmount)
    {
      if (this._applyonethroughtwoFUCKYOUTEVLEVTHUMBSDOWNEMOJI)
        entryVariable = UnityEngine.Random.Range(1, 2);
      ...
      int defense = entryVariable;
      if (this._useRandomBetweenPrevious)
        defense = UnityEngine.Random.Range(this.PreviousExitValue, entryVariable + 1);
      bool flag2;
      if (flag1)
      {
        flag2 = false;
      ...
      else
      {
        ...
        {
          if (CombatManager.Instance._stats.combatSlots.SlotContainsSlotStatusEffect(targets[index1].SlotID, targets[index1].IsTargetCharacterSlot, (SlotStatusEffectType) 866795))
          {
